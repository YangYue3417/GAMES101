...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    int max_x = std::max(t.v[0][0], std::max(t.v[1][0], t.v[2][0]));
    int min_x = std::min(t.v[0][0], std::min(t.v[1][0], t.v[2][0]));
    int max_y = std::max(t.v[0][1], std::max(t.v[1][1], t.v[2][1]));
    int min_y = std::min(t.v[0][1], std::min(t.v[1][1], t.v[2][1]));
    ...
    for (int x = min_x; x <= max_x; x++)
    {
        for (int y = min_y; y <= max_y; y++)
        {
            if (insideTriangle(x, y, t.v))
            {
                ...
                std::tie(alpha, beta, gamma) = computeBarycentric2D((float)x + 0.5f, (float)y + 0.5f, t.v);

                // float Z = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                // float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                // zp *= Z;
                float Z = 1.0 / (static_cast<double>(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w()));
                float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                zp *= Z;
...
{
    frame_buf.resize(static_cast<double>(w * h));
    depth_buf.resize(static_cast<double>(w * h));
